...
void setMotorPWM(DC_motor *m)
{
    ...
    if(m->brakemode) {
        posDuty=m->PWMperiod - ((unsigned char)(m->power)*(m->PWMperiod))/100;
        negDuty=m->PWMperiod;
    }
    else {
        posDuty=0;
  negDuty=((unsigned char)(m->power)*(m->PWMperiod))/100;
    }
...
void goStraight(DC_motor *mL, DC_motor *mR, unsigned int direction, int squares)
{
    ...
    for(int i =0; i< squares; i++)
    {
        ...
        _delay((unsigned long)((850)*(64000000/4000.0)));
