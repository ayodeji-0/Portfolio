...
# 73 "./color.h"
        color red = {'r',6615,5380,775,980,2, 2, 0,0};
        color green = {'g',8355,3303,3540,1740, 2, 2, 1,0};
        color blue = {'b',2975,1032,995,960, 2, 4, 0,0};
        ...
        color whiteC = {'w',16230,7745,5360,3860,2, 0, 0,0};
        color blackC = {'k',1592,760,467,332,2, 0, 0,0};
...
void color_read_all(RGBC *readVals){
 ...
 tmp.red=I2C_2_Master_Read(1);
 tmp.red=tmp.red | (I2C_2_Master_Read(1)<<8);
 tmp.green=I2C_2_Master_Read(1);
 tmp.green=tmp.green | (I2C_2_Master_Read(1)<<8);
 tmp.blue=I2C_2_Master_Read(1);
 tmp.blue=tmp.blue | (I2C_2_Master_Read(1)<<8);
 tmp.clear=I2C_2_Master_Read(1);
 tmp.clear=tmp.clear | (I2C_2_Master_Read(0)<<8);
 I2C_2_Master_Stop();
...
void checkWhite(void){
 ...
 if(tmp.clear == calibColors[7].clear && tmp.red == calibColors[7].red && tmp.green == calibColors[7].green && tmp.blue == calibColors[7].blue ){
  finished = 1;
 }
 else{
  finished = 0;
...
void color2instruction(DC_motor mL,DC_motor mR,color currColor,colorHistory *hist){
 ...
 if(!finished){
  ...
  if(readingColor == 1){
   ...
   storeSequence(&hist,currColor.angle,currColor.polarity,currColor.direction,squaresb4,currColor.squares);
...
void processSequence(colorHistory *history){
 ...
 for(int i = 0; i < pos; i++){
  history -> polarity[i] = !history -> polarity[i];
        if(history -> direction[i] != 2){
            history ->direction[i] = !history -> direction[i];
        }
...
void useSequence(DC_motor mL, DC_motor mR, colorHistory *history){
 ...
 for(int i = pos; i > 0; i--){
   goStraight(&mL,&mR,history -> direction[i-1],history -> squaresb4[i-1]);
   _delay((unsigned long)((1000)*(64000000/4000.0)));
   turn(mL,mR,history -> angle[i-1],history -> polarity[i-1]);
   _delay((unsigned long)((1000)*(64000000/4000.0)));
   goStraight(&mL,&mR,history -> direction[i-1],history -> squares[i-1]);
  }
   stop(&mL,&mR);
...
void away(DC_motor mL,DC_motor mR, colorHistory *hist){
 ...
 color2instruction(mL,mR,currColor,hist);
